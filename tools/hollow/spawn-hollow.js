...
async function doSpawn(fileName) {
  const CODE = fs.readFileSync(`./lua/hollow/${fileName}`, 'utf-8');
  const signer = createDataItemSigner(WALLET);

  console.info(`Spawning AOS ${fileName} Lua process`);
  const processId = 'iWM-odlyQHopPECpyz465p7ED8lm5d3hyyWtijKhie4';
  ...
  try {
    ...
    fs.writeFileSync(`./tools/hollow/hollow-id.txt`, processId, 'utf-8');
  } catch (e) {
    console.error(e);
  }
...
doSpawn('hollow.lua').then((processId) => console.log(`https://www.ao.link/#/entity/${processId}`));
