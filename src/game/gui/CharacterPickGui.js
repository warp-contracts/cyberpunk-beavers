...
function CharacterOption() {
  return {
    view: function (vnode) {
      ...
      return m('.character', { onclick: () => changeScene(character) }, [
          ...
          m('.character-box-container', [
            m('.character-box-interrior', [
              ...
              m('.d-flex flex-column', [
                m(CharacterAbility, { name: 'AP', value: stats.ap.max }),
                m(CharacterAbility, { name: 'HP', value: stats.hp.max }),
                m(CharacterAbility, {
                  name: 'CBTOKEN BONUS',
              ...
              m('.d-flex flex-column', [
                ...
                m('.d-flex character-ability', [m('div', 'RANGE')]),
                m(
...
function CharacterAbility() {
  return {
    view: function (vnode) {
      ...
      return m('.character-ability d-flex justify-between', [m('div', name), m('div', value)]);
    },
  };
}
...
function CharacterBoxWithCorners() {
  return {
    view: function (vnode) {
      const { damage, hitChance, criticalHitChance, criticalHitMultiplier } = vnode.attrs;
      return m('.character-box-with-corners', [
        m('.corner.top-left', [`${damage.value}HP`, m('span.hint', damage.hint)]),
