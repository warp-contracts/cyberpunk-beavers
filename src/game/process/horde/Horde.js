...
export class Horde {
  ...
  _moveMonsters(msgTimestamp) {
    ...
    for (let i = 0; i < recoveredMonsters.length; i++) {
      ...
      // process possible attacks generated by the player's mine that the monster have stepped onto after 'Brain._move'
      // note: the 'attack' client updates from the mines MUST be added after updates from 'Brain._move'
      if (this.playerAttack) {
        this.processPlayerAttack();
      }
